...
import com.pcitc.htmltopdf.service.PrintTempManager;
...
@Service
public class PrintTempManagerImpl implements PrintTempManager {
	// TODO 共用sql 提取
	...
	public Map<String, Object> getTableHtml(Map<String, Object> paramMap){
		...
		// 0.模板数据
		String printSql = "select * from PRINT_TEMP  where PRINT_TEMP ID = '" + printTempId + "' ";
	...
	public List<Map<String, Object>> getMorePrintData(List<Map<String, Object>> paramMap) {
		List<Map<String, Object>> tableArray = new ArrayList<>();
		try {
			paramMap.forEach(stringObjectMap -> {
...
//			String[] fieldSources = request.getParameterValues("fieldSource");
	 ...
	 * 
	 * 打印模板格式大致分三种类型 
	 * 	 不同的类型说明见 PrintTempManagerImpl 开始部分
	 */
	@Override
	public Map<String, Object> getSinglePrintData(Map<String, Object> paramMap)  {
		...
		try {
			if(paramMap.get("printTempId") == null){
				throw new RuntimeException("模板ID为必填项");
			}
			String printTempId = paramMap.get("printTempId").toString();
